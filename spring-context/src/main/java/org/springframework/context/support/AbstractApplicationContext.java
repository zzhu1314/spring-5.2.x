...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			* 		b、加载spring所有jar中的spring.handlers文件(SPI机制 spring provider interfice  读取配置文件（key=value）)。并建立映射关系
			...
			* */
			...
			try {
				...
				//完成refresh()后触发ContentRefreshedEvent
				finishRefresh();
			}
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		//模板设计模式，父类定义主流程，抽象方法(钩子函数)交给具体子类实现
		refreshBeanFactory();
		return getBeanFactory();
	}
	...
	protected void initApplicationEventMulticaster() {
		...
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
			...
			//applicationEventMulticaster将注到容器
	...
	protected void registerListeners() {
		...
		//一般容器启动时 容器的applicationListeners 为空，需要调用addApplicationListener()
